...
import requests
import json
from ..utils.funcs import validate_number, get_valid_number, min_duration, max_active_groups
from ..utils.funcs import DEFAULT_MIN_DURATION
import inspect
...
class UltramsgInstance(models.Model):
    ...
    linked_name = fields.Char(compute="_linked_comp",
                              help='Name of linked whatsapp account')
    linked_number = fields.Char(
        compute="_linked_comp", help='Number of linked whatsapp account')
    instance_id = fields.Char(string="Instance ID",
                              required=True, help='Ultramsg instance id')
    ...
    send_group_ids = fields.One2many(
        'ultramsg.send_group', 'instance_id', string='Groups')
    message_report_ids = fields.One2many(
        'ultramsg.message_report', 'instance_id', string='Sent Report')
...
class UltramsgMobileNo(models.Model):
    ...
    send_group_ids = fields.Many2many(
        'ultramsg.send_group', string='Send Groups')
    message_report_ids = fields.One2many(
        'ultramsg.message_report', 'mobile_no_id', string='Sent Report')
    ...
    def write(self, vals):
        if 'number' in vals:
            vals['number'] = validate_number(vals['number'])
        return super(UltramsgMobileNo, self).write(vals)
    ...
    def send_text(self, instance, message, group=None):
        ...
        if instance.status != 'connected':
            ...
            response = get_ultramsg_data(
                url_extra, data=payload, method="POST")
            result = response.json()

            if not result.get('error') and result.get('sent') == 'true' and result.get('message') == 'ok':
                status = "delivered"

        self.env['ultramsg.message_report'].sudo().create({
...
class UltramsgSendGroup(models.Model):
    ...
    duration = fields.Float(
        'Duration HH:mm', default=min_duration, help="Durations between each message")
    iterations = fields.Integer(string='Iterations', default=1,
                                help="Count of send iterations: '0' for infinite iterations")
    ...
    message_id = fields.Many2one(
        string='Message', comodel_name='ultramsg.message', ondelete='set null')
    message_body = fields.Text(
        related='message_id.body', string='Message Body')
    instance_id = fields.Many2one(
        string='Instance', comodel_name='ultramsg.instance', ondelete='set null')
    is_sendable = fields.Boolean(compute='_compute_is_sendable')
    cron_id = fields.Integer()
    instance_is_connected = fields.Boolean(
        compute='_compute_instance_is_connected')
    message_report_ids = fields.One2many(
        'ultramsg.message_report', 'send_group_id', string='Sent Report')

    @api.model
    def create(self, vals):
        return super().create(self._validate(vals))

    def write(self, vals):
        rec = super(UltramsgSendGroup, self).write(self._validate(vals))
    ...
    def _validate(self, vals):
        # checkin max active value ahndled on change
        if 'iterations' in vals:
            vals['iterations'] = max(vals['iterations'], 0)

        if 'current_iteration' in vals:
            vals['current_iteration'] = max(vals['current_iteration'], 1)

        if 'duration' in vals and vals['duration'] < min_duration(self):
            raise ValidationError(
                _("Duration can't be less than %.2f", min_duration(self)))
            # cron.interval_number = self.duration
        return vals

    @api.depends('mobile_no_ids', 'message_id', 'instance_id')
    def _compute_is_sendable(self):
        for rec in self:
            rec.is_sendable = bool(
                rec.instance_id and rec.message_id and rec.mobile_no_ids)
    ...
    def _compute_sent_to(self):
        for rec in self:
            html = '''<table class="o_list_table table table-sm table-hover table-striped o_list_table_ungrouped"
    style="table-layout: fixed;">
    <thead>
        <tr>
            <th data-name="number" tabindex="-1" class="o_column_sortable" data-original-title="" title=""
                style="width: 451px;">Number<span class="o_resize"></span></th>
            <th data-name="name" tabindex="-1" class="o_column_sortable" data-original-title="" title=""
                style="width: 267px;">Name<span class="o_resize"></span></th>
        </tr>
    </thead>
    <tbody class="ui-sortable">'''
            html += ''.join(
                [f'<tr class="o_data_row" data-id="ultramsg.moblie_no_6"><td class="o_data_cell o_field_cell o_list_char o_required_modifier" tabindex="-1" title="{x.number}">{x.number}</td><td class="o_data_cell o_field_cell o_list_char" tabindex="-1" title="{x.name}">{x.name}</td></tr>' for x in rec.sent_to_ids])
            html += '''        <tr>
            <td colspan="2">&nbsp;</td>
        </tr>
        <tr>
    ...
    def onchange_is_active(self):
        if self.is_active and not self._origin.is_active:
            if self.sent_to:
                title = _("Warning")
                message = _(
                    "The group will be activated with these settings:") + "\n"
                message += "    ●" + "    " + \
                    _("Iterations: ") + str(self.iterations) + "\n"
                message += "    ●" + "    " + \
                    _("Current iteration: ") + \
            ...
            if self.n_active >= max_active_groups(self):
                title = _('Error')
                message = _(
                    "Active groups reached the maximum allowed, disable a group or contact the Administrator for more")
    ...
    def action_group_send(self, rec_id):
        ...
        sent, instance_connected = mob.send_text(
            rec.instance_id, rec.message_body, rec)
...
class UltramsgMessageReport(models.Model):
    ...
    mobile_no_id = fields.Many2one(
        string='To', comodel_name='ultramsg.moblie_no', ondelete='set null')
    send_group_id = fields.Many2one(
        string='Send Group', comodel_name='ultramsg.send_group', ondelete='set null')
    instance_id = fields.Many2one(
        string='Instance', comodel_name='ultramsg.instance', ondelete='set null')
...
class TextMessage(models.Model):
    ...
    send_group_ids = fields.One2many(
        'ultramsg.send_group', 'message_id', string='Send Groups')
...
class ResPartner(models.Model):
    ...
    def _compute_ultramsg_number(self):
        for record in self:
            record.ultramsg_number = get_valid_number(
                self.phone or self.mobile or '')
